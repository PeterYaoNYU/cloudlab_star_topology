...
central_node.addService(pg.Execute(shell="sh", command="sysctl -w net.ipv4.ip_forward=1"))
...
for i in range(1, num_spokes + 1):
    ...
    iface = node.addInterface(f"ifc_node{i}")
    iface.addAddress(pg.IPv4Address(f"10.0.0.{i}", "255.255.255.0"))
    ...
    node.addService(pg.Execute(shell="sh", command="sysctl -w net.ipv4.ip_forward=1"))
    
    # Add static route on the spoke node to reach other spoke nodes via the central router.
    for j in range(1, num_spokes + 1):
        if i != j:
            node.addService(pg.Execute(shell="sh", command=f"ip route add 10.0.0.{j} via 10.0.0.100"))
